...
Coloque os seus arquivos XML de notas fiscais (ex: `DANFE.xml` ou `NotaServico.xml`) na mesma pasta do script (ou configure o caminho da pasta no código).
Execute o script principal:
```bash
   python main.py
   ```
O script irá processar os arquivos e gerar automaticamente as planilhas de saída (ex: `NFsSP.xlsx` e `NFsRJ.xlsx`) no mesmo diretório, contendo todos os dados estruturados e prontos para uso.
